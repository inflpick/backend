...
public interface QueryInfluencerControllerDocs {
    ...
    @Operation(summary = "인플루언서 단건 조회", description = "인플루언서를 단건 조회합니다.",
            ...
            responses = {
                    @ApiResponse(responseCode = "200", description = "성공", content = @Content(schema = @Schema(implementation = QueryInfluencerResponse.class))),
            }
    )
    ResponseEntity<QueryInfluencerWebResponse> get(@PathVariable String id);
    ...
    ResponseEntity<PageWebResponse<QueryInfluencerWebResponse>> list(@RequestParam(name = "page", required = false, defaultValue = "0") Integer page,
