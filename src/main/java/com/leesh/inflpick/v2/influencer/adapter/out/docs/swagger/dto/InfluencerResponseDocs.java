...
import com.leesh.inflpick.v2.influencer.application.dto.SnsProfileLinkResponse;
...
public interface InfluencerResponseDocs {
    ...
    @ArraySchema(schema = @Schema(description = "소셜 미디어 링크 목록", implementation = SnsProfileLinkResponseDocs.class, requiredMode = Schema.RequiredMode.REQUIRED))
    List<SnsProfileLinkResponse> socialMediaProfileLinks();

    @ArraySchema(schema = @Schema(description = "인물에 대한 키워드 목록", implementation = KeywordResponseDocs.class, requiredMode = Schema.RequiredMode.REQUIRED))
